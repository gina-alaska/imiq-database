...
--   sourceIDs: 248 to 258, VariableID: 1045
  IF EXISTS (SELECT * FROM tables.odmdatavalues_metric WHERE siteid = $1 and $2 = 370) OR
     EXISTS (SELECT * FROM tables.odmdatavalues_metric WHERE siteid = $1 and $2 = 75) OR
     EXISTS (SELECT * FROM tables.odmdatavalues_metric WHERE siteid = $1 and $2 = 230) OR
     EXISTS (SELECT * FROM tables.odmdatavalues_metric WHERE siteid = $1 and $2 = 1045) 
  THEN
    OPEN loopCursor
...
--   SourceID = 213, VaraibleID = 584
  ELSIF EXISTS (SELECT * FROM tables.odmdatavalues_metric WHERE siteid = $1 and $2 = 813) OR
        ...
        EXISTS (SELECT * FROM tables.odmdatavalues_metric WHERE siteid = $1 and $2 = 584) 
  THEN
    OPEN loopCursor
    for execute format('SELECT date_trunc(''hour'', dv.datetimeUTC) as DateTimeUTC,AVG(dv.DataValue) FROM tables.ODMDataValues_metric AS dv '
		         'WHERE dv.SiteID = $1 and dv.OriginalVariableid=$2 GROUP BY date_trunc(''hour'', dv.datetimeUTC);') using site_id, var_id;
