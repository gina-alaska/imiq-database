...
/****** Object:  StoredProcedure [dbo].[KUPARUK_uspGetDailyRH_4]    Script Date: 02/20/2013 12:54:18 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[KUPARUK_uspGetDailyRH_4] 
...
BEGIN
    ...
    BEGIN
		...
		WHERE dv.SiteID = @SiteID and dv.VariableID=80 and convert(int,datepart(hh,dv.LocalDateTime)) = 4  and (dv.offsetvalue = 2 or dv.offsetvalue = 1.5) and dv.siteid in 
	    ...
	    BEGIN
	        INSERT INTO KUPARUK_DailyRH4DataValues (DataValue,UTCDateTime, VariableID,SiteID,MethodID)
    ...
    ELSE IF EXISTS (SELECT * FROM ODMDataValues WHERE SiteID= @SiteID AND VariableID=80 AND OffsetValue = 1)
     BEGIN
          DECLARE max_cursor CURSOR FOR 
              SELECT CONVERT(Date,dv.DateTimeUTC), AVG(dv.DataValue)
              FROM ODMDataValues AS dv
              WHERE dv.SiteID = @SiteID and dv.VariableID=80 and convert(int,datepart(hh,dv.LocalDateTime)) = 4  and dv.offsetvalue = 1 and dv.siteid in 
              (select distinct siteid from seriesCatalog where @SiteID = siteid)
              GROUP BY CONVERT(Date,dv.DateTimeUTC);
    ...
    ELSE IF EXISTS (SELECT * FROM ODMDataValues WHERE SiteID= @SiteID AND VariableID=467 AND OffsetValue = 1)
     BEGIN
          DECLARE max_cursor CURSOR FOR 
              SELECT CONVERT(Date,dv.DateTimeUTC), AVG(dv.DataValue)
              FROM ODMDataValues AS dv
              WHERE dv.SiteID = @SiteID and dv.VariableID=467 and convert(int,datepart(hh,dv.LocalDateTime)) = 4 and dv.offsetvalue = 1 and dv.siteid in 
              (select distinct siteid from seriesCatalog where @SiteID = siteid)
              GROUP BY CONVERT(Date,dv.DateTimeUTC)
