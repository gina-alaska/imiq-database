...
/****** Object:  StoredProcedure [dbo].[uspGetHourlyWindSpeed]    Script Date: 01/09/2015 13:49:23 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
    ...
    -- VariableID = 82. SourceID = 29,30,31,34,223
   ...
   ELSE IF EXISTS (SELECT * FROM seriesCatalog WHERE SiteID= @SiteID AND @VarID=469 )
    ...
    -- VariableID = 827, SourceID = 144
   ELSE IF EXISTS (SELECT * FROM seriesCatalog WHERE SiteID= @SiteID AND @VarID=827 )
    BEGIN
	    DECLARE max_cursor CURSOR FOR 
		select DateAdd(hh, DATEPART(hh, WS.DateTimeUTC), DateAdd(d, DateDiff(d, 0, WS.DateTimeUTC), 0)) as DateTimeUTC,AVG(DataValue),MAX_Offset.max_OffsetValue,MAX_Offset.max_OffsetTypeID
          from ODMDataValues_metric WS
          inner join
           (select SiteID,DateTimeUTC,MAX(OffsetValue) as max_OffsetValue,OffsetTypeID as max_OffsetTypeID
            from ODMDataValues_metric 
            ...
            on MAX_Offset.SiteID=WS.SiteID and MAX_Offset.DateTimeUTC=WS.DateTimeUTC 
            WHERE WS.SiteID = @SiteID and OriginalVariableid=@VarID and WS.OffsetValue=MAX_Offset.max_OffsetValue
            GROUP BY DateAdd(hh, DATEPART(hh, WS.DateTimeUTC), DateAdd(d, DateDiff(d, 0, WS.DateTimeUTC), 0)),MAX_Offset.max_OffsetValue,MAX_Offset.max_OffsetTypeID
	...
	-- AON  Windspeed/ms, AST
	-- using gap-filled wind speed
    -- VariableID = 815, SourceID = 222
 ELSE IF EXISTS (SELECT * FROM seriesCatalog WHERE SiteID= @SiteID AND @VarID=815 )
    BEGIN
	    DECLARE max_cursor CURSOR FOR 
