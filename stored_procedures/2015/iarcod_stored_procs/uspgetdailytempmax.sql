...
/****** Object:  StoredProcedure [dbo].[uspGetDailyTempMax]    Script Date: 02/20/2013 12:59:27 ******/
SET ANSI_NULLS ON
GO
...
-- Description:	Create the daily maximum air temperatures 
-- NOTE: This table that is holding the max AirTemp data values has been deleted, since it was run with an old verion
-- of the seriesCatalog.  If a new version is needed,run again.
-- (Nov 11,2012).
-- =============================================
CREATE PROCEDURE [dbo].[uspGetDailyTempMax] 
...
BEGIN
    ...
    BEGIN
		...
		SELECT CONVERT(Date,dv.DateTimeUTC), MAX(dv.DataValue)
	    ...
	    BEGIN
	        INSERT INTO DailyTempMaxDataValues (DataValue,UTCDateTime, VariableID,SiteID,MethodID)
