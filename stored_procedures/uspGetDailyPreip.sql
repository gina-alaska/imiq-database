...
/****** Object:  StoredProcedure [dbo].[uspGetDailyPrecip]    Script Date: 01/09/2015 13:45:09 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
    ...
    BEGIN
	    ...
	    BEGIN
	        INSERT INTO DAILY_PrecipDataValues (DataValue,UTCDateTime,SiteID,OriginalVariableID,InsertDate)
	        VALUES(@avgValue, @DateTimeUTC, @SiteID, 678,GETDATE());
			FETCH NEXT FROM max_cursor INTO @DateTimeUTC, @avgValue;
        END
	    CLOSE max_cursor;
		DEALLOCATE max_cursor;
    ...
    ELSE IF EXISTS (SELECT * FROM seriesCatalog WHERE SiteID= @SiteID AND @VarID=839)
    BEGIN
	    DECLARE max_cursor CURSOR FOR 
