...
/****** Object:  StoredProcedure [dbo].[uspGetHourlyPrecip]    Script Date: 02/07/2014 11:25:15 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
    ...
    IF EXISTS (SELECT * FROM seriesCatalog_62 WHERE SiteID= @SiteID AND @VarID=340)
    BEGIN
    ...
    ELSE IF EXISTS (SELECT * FROM seriesCatalog_62 WHERE SiteID= @SiteID AND @VarID=319)
    BEGIN
	    DECLARE max_cursor CURSOR FOR 
		SELECT DateAdd(hh, DATEPART(hh, DateTimeUTC), DateAdd(d, DateDiff(d, 0, DateTimeUTC), 0)) as DateTimeUTC,MAX(dv.DataValue)
		FROM ODMDataValues_metric AS dv
    ...
    ELSE IF EXISTS (SELECT * FROM seriesCatalog_62 WHERE SiteID= @SiteID AND @VarID=461)
    BEGIN
	    DECLARE max_cursor CURSOR FOR 
		SELECT DateAdd(hh, DATEPART(hh, DateTimeUTC), DateAdd(d, DateDiff(d, 0, DateTimeUTC), 0)) as DateTimeUTC,MAX(dv.DataValue)
		FROM ODMDataValues_metric AS dv
    ...
    -- VariableID = 496 
    ELSE IF EXISTS (SELECT * FROM seriesCatalog_62 WHERE SiteID= @SiteID AND @VarID=496)
    BEGIN
	    DECLARE max_cursor CURSOR FOR 
		SELECT DateAdd(hh, DATEPART(hh, DateTimeUTC), DateAdd(d, DateDiff(d, 0, DateTimeUTC), 0)) as DateTimeUTC,MAX(dv.DataValue)
		FROM ODMDataValues_metric AS dv
  ...
  -- BLM/Kemenitz. Precip/inches/every 15 minutes  SourceID = 199
    ...
    ELSE IF EXISTS (SELECT * FROM seriesCatalog_62 WHERE SiteID= @SiteID AND @VarID=458)
    BEGIN
	    DECLARE max_cursor CURSOR FOR 
...
-- VariableID = 336
-- SUM, not max, since the DataValue is not accumulating over the hour.
  ELSE IF EXISTS (SELECT * FROM seriesCatalog_62 WHERE SiteID= @SiteID AND @VarID=336)
    BEGIN
	    DECLARE max_cursor CURSOR FOR 
		SELECT DateAdd(hh, DATEPART(hh, DateTimeUTC), DateAdd(d, DateDiff(d, 0, DateTimeUTC), 0)) as DateTimeUTC,SUM(dv.DataValue)
		FROM ODMDataValues_metric AS dv
...
-- NOAA. Precip/mm/Minute  SourceID = 35
    -- VariableID = 522 
    -- These values are all NULL, as of 6/6/2013
  ELSE IF EXISTS (SELECT * FROM seriesCatalog_62 WHERE SiteID= @SiteID AND @VarID=522)
    BEGIN
	    DECLARE max_cursor CURSOR FOR 
		SELECT DateAdd(hh, DATEPART(hh, DateTimeUTC), DateAdd(d, DateDiff(d, 0, DateTimeUTC), 0)) as DateTimeUTC,SUM(dv.DataValue)
		FROM ODMDataValues_metric AS dv
...
-- SUM, not max, since the DataValue is not accumulating over the hour.
  ELSE IF EXISTS (SELECT * FROM seriesCatalog_62 WHERE SiteID= @SiteID AND @VarID=539)
    BEGIN
	    DECLARE max_cursor CURSOR FOR 
		SELECT DateAdd(hh, DATEPART(hh, DateTimeUTC), DateAdd(d, DateDiff(d, 0, DateTimeUTC), 0)) as DateTimeUTC,SUM(dv.DataValue)
		FROM ODMDataValues_metric AS dv
...
-- Need to compute hourly
  ELSE IF EXISTS (SELECT * FROM seriesCatalog_62 WHERE SiteID= @SiteID AND @VarID=526)
    BEGIN
	    DECLARE max_cursor CURSOR FOR 
  ...
  ELSE IF EXISTS (SELECT * FROM seriesCatalog_62 WHERE SiteID= @SiteID AND @VarID=294)
    BEGIN
	    DECLARE max_cursor CURSOR FOR 
		SELECT DateAdd(hh, DATEPART(hh, DateTimeUTC), DateAdd(d, DateDiff(d, 0, DateTimeUTC), 0)) as DateTimeUTC,MAX(dv.DataValue)
		FROM ODMDataValues_metric AS dv
  ...
  ELSE IF EXISTS (SELECT * FROM seriesCatalog_62 WHERE SiteID= @SiteID AND @VarID=575)
    BEGIN
	    DECLARE max_cursor CURSOR FOR 
		SELECT DateAdd(hh, DATEPART(hh, DateTimeUTC), DateAdd(d, DateDiff(d, 0, DateTimeUTC), 0)) as DateTimeUTC,SUM(dv.DataValue)
		FROM ODMDataValues_metric AS dv
