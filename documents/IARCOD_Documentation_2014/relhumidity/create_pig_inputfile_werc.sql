...
where OriginalVariableID=81 and OffsetValue=3) AS D_AT3
on D_AT1.SiteID=D_AT3.SiteID and D_AT1.DateTimeUTC=D_AT3.DateTimeUTC
left outer join
(select SiteID,DateTimeUTC,DataValue as RH1
from ODMDataValues_metric
where OriginalVariableID=80 and OffsetValue=1 ) as D_RH1
 on D_RH1.SiteID=D_AT1.SiteID and D_RH1.DateTimeUTC=D_AT1.DateTimeUTC
left outer join
(select SiteID,DateTimeUTC,DataValue as RH3
from ODMDataValues_metric
where OriginalVariableID=80 and OffsetValue=3 ) as D_RH3
 on D_RH3.SiteID=D_AT3.SiteID and D_RH3.DateTimeUTC=D_AT3.DateTimeUTC
where D_AT1.SiteID in (select siteid from sites where sourceid in(29,30,31,34)) and D_AT1.OriginalVariableID=81 and D_AT1.OffsetValue=1
union
...
from ODMDatavalues_metric
where OriginalVariableID=81 and OffsetValue=1) AS D_AT1
...
where OriginalVariableID=80 and OffsetValue=3 ) as D_RH3
 on D_RH3.SiteID=D_AT3.SiteID and D_RH3.DateTimeUTC=D_AT3.DateTimeUTC
where D_AT3.SiteID in (select siteid from sites where sourceid in(29,30,31,34)) and D_AT3.OriginalVariableID=81 and D_AT3.OffsetValue=3
