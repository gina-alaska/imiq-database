...
from DAILY_RH
where OriginalVariableID=293) AS RH  /* keep consistent with AT, using OriginalVariableID */
on AT.SiteID=RH.SiteID and AT.UTCDateTime=RH.UTCDateTime
where AT.SiteID in (select siteid from sites where sourceid in(182)) and AT.OriginalVariableID=279  /* Need to use OriginalVariableID, since multiple ATs */
