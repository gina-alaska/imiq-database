...
--   SourceID = 164, VariableID = 152
  ...
  THEN
    OPEN max_cursor
    for execute format('SELECT date_trunc(''day'',dv.loacldatetime) as DateTimeUTC, dv.DataValue FROM tables.odmdatavalues_metric AS dv '
...
--   SourceID = 31, VariableID = 150
  ELSIF EXISTS (SELECT * FROM tables.odmdatavalues_metric WHERE siteid = $1 AND $2 = 445) OR 
        ...
        EXISTS (SELECT * FROM tables.odmdatavalues_metric WHERE siteid = $1 AND $2 = 150)
  THEN
    OPEN max_cursor
    for execute format('SELECT  date_trunc(''day'',dv.datetimeutc) as DateTimeUTC, AVG(dv.DataValue) FROM tables.odmdatavalues_metric AS dv '
                         'WHERE dv.siteid = $1 and dv.originalvariableid=$2 '
                         'GROUP BY date_trunc(''day'',dv.datetimeutc);') USING site_id,var_id;
        loop
        fetch max_cursor into DateTimeUTC, avgValue;
	  if not found then
	    exit;
