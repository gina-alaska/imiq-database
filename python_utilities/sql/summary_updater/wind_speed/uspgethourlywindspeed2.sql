...
--   SourceID = 144, VariableID = 827
  ...
  THEN
    OPEN loopCursor
    for execute format('select date_trunc(''hour'', ws.datetimeUTC) as DateTimeUTC,
                                AVG(DataValue),
                                MAX_Offset.max_OffsetValue,
                                MAX_Offset.max_OffsetTypeID from tables.ODMDataValues_metric WS
                        inner join
                            (select SiteID,
                                    DateTimeUTC,
                        ...
                        GROUP by date_trunc(''day'',ws.datetimeutc), MAX_Offset.max_OffsetValue, MAX_Offset.max_OffsetTypeID;')using site_id, var_id;
        loop
